...
class Magnet():
  ...
  __modifier=dict()
  __tXtranslation=np.array([[-1,0,0],[1,0,0]])*__displacement
  __tXrotation=np.tile(np.expand_dims(np.eye(3),0),(2,1,1))
  __tYtranslation=np.array([[0,-1,0],[0,1,0]])*__displacement
  __tYrotation=np.tile(np.expand_dims(np.eye(3),0),(2,1,1))
  __tZtranslation=np.array([[0,0,-1],[0,0,1]])*__displacement
  ...
  def genCache(self,fileName,parameters,override=False):
    ...
    for z in np.linspace(self.__z_offset,self.__z_offset+self.__z_step*self.__z_count,self.__z_count,endpoint=False):
      for x in np.linspace(self.__x_offset,self.__x_offset+self.__x_step*self.__x_count,self.__x_count,endpoint=False):
  ...
  def Linearization(self,z,x,y=0,preTiltAngle=0,operations=['tX','tY','tZ','rX','rY'],floatMagnet=None):
    ...
    for operation in operations:
      ...
      if operation=='rX' or operation=='rY':
        ...
        temp/=2*self.__displacement
      A.append(temp.copy())
    A=np.array(A).T # acc unit is mm/s^2, angular acc unit is 1/s^2
    return oneOverMassAndInertia@A
  
  def LinearizationA(self,z,x,y=0,preTiltAngle=0,floatMagnet=None,symmetry=True):
    ...
    if (not symmetry) or (x==0 and y==0):
  ...
  def LinearizationB(self,z,x,y=0,preTiltAngle=0,floatMagnet=None,symmetry=True):
    B1=self.forceAndTorque(z,x,y,preTiltAngle,floatMagnet=floatMagnet)[:-1]
    if isinstance(floatMagnet,Magnet):
      magnetMass=floatMagnet.Mass
      magnetInertia=floatMagnet.Inertia
